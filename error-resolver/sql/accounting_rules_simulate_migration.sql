...
SELECT 'Sequence' AS TYPE, ACCOUNTING_RULES_SEQ.CURRVAL AS VALUE FROM DUAL
UNION ALL
SELECT 'Max ObjectID', MAX(OBJECT_ID) FROM ACCOUNTING_RULES;
...
-- Verify the gap: sequence = 5, max OBJECT_ID = 1007 (OUT OF SYNC)
...
SELECT 'Sequence (stuck at)' AS TYPE, ACCOUNTING_RULES_SEQ.CURRVAL AS VALUE FROM DUAL
UNION ALL
SELECT 'Max ObjectID (jumped to)', MAX(OBJECT_ID) FROM ACCOUNTING_RULES;
